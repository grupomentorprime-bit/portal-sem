/**
 * OT-GROWTH-MESSAGING-005 — capturas Ajustes → Canales (desktop + móvil).
 * Preferencia: Espacio ADL (`adl`).
 * Uso: npm run dev → npx tsx --env-file=.env scripts/capture-growth-messaging-005.ts
 *
 * Entrega: canales desktop · WhatsApp conectado · incompleto/pausado · móvil.
...
const outDir = resolve("docs/validation/OT-GROWTH-MESSAGING-005");
mkdirSync(outDir, { recursive: true });

const CAPTURE_TAG = "capture-messaging-005";
...
async function main() {
  ...
  // —— Desktop: conectado ——
  {
    ...
    await upsertCaptureConnection(db, tenantId, {
      enabled: false,
      hasAccessToken: true,
      displayPhoneNumber: "+56 9 1111 0000",
      phoneNumberId: `capture-pn-${tenantId}-paused`,
    });
    await page.reload({ waitUntil: "load", timeout: 180000 });
    await hideDevOverlay(page);
    await page.waitForSelector('[data-status="paused"]', { timeout: 30000 });
    await page.waitForTimeout(400);
    await page.screenshot({
      path: resolve(outDir, "admin-channels-whatsapp-paused.png"),
      fullPage: true,
    });
    ...
    console.log("saved paused + incomplete");

    await context.close();
  }

  // —— Mobile: no re-capturado en 005A (entrega solo desktop) ——
  console.log("skip mobile (005A)");
