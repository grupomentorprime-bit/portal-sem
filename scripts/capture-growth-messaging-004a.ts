/**
 * OT-GROWTH-MESSAGING-004A — capturas refinamiento visual Mensajes.
 * Preferencia: Espacio ADL (`adl`).
 * Uso: npm run dev → npx tsx --env-file=.env scripts/capture-growth-messaging-004a.ts
 *
 * Entrega: desktop bandeja · desktop escribiendo · móvil lista · móvil conversación.
...
const outDir = resolve("docs/validation/OT-GROWTH-MESSAGING-004A");
mkdirSync(outDir, { recursive: true });

const CAPTURE_TAG = "capture-messaging-004a";
...
async function main() {
  ...
  // Limpia semillas de captura 004 / 004A para evidencia visual limpia.
  for (const tag of [CAPTURE_TAG, "capture-messaging-004"]) {
    const prevPersonas = await db
      .collection("growth_personas")
      .find({ tenantId, "origin.sourceId": tag })
      .project({ _id: 1 })
      .toArray();
  ...
  await db.collection("growth_mensajes").deleteMany({
    tenantId,
    externalMessageId: {
      $regex: `^capture(-(004a)?-)?(in|out)-|^capture-004a-(in|out)-`,
    },
  });
  await db.collection("growth_conversaciones").deleteMany({
    tenantId,
    externalThreadId: { $regex: `^capture(-004a)?-phone:` },
  });
  ...
  await db.collection("growth_personas").insertMany([
    {
      ...
      firstName: "María",
      lastName: "González",
      email: `maria.messaging004a.${stamp}@example.com`,
      emailNormalized: `maria.messaging004a.${stamp}@example.com`,
    ...
    },
    {
      ...
      firstName: "Carlos",
      lastName: "Pérez",
      email: `carlos.messaging004a.${stamp}@example.com`,
      emailNormalized: `carlos.messaging004a.${stamp}@example.com`,
  ...
  await db.collection("growth_conversaciones").insertMany([
    {
      ...
      externalThreadId: `capture-004a-phone:56987654321`,
      createdAt: t1,
      updatedAt: t5,
      lastMessageAt: t5,
    },
    {
      ...
      channel: "whatsapp",
      status: "open",
      externalThreadId: `capture-004a-phone:56912345678`,
      createdAt: t3,
      updatedAt: t3,
      lastMessageAt: t3,
    },
  ...
  await db.collection("growth_mensajes").insertMany([
    {
      ...
      status: "received",
      externalMessageId: `capture-004a-in-a1-${stamp}`,
      clientRequestId: `capture-004a-req-a1-${stamp}`,
      occurredAt: t1,
      createdAt: t1,
    },
    {
      ...
      status: "sent",
      externalMessageId: `capture-004a-out-a2-${stamp}`,
      clientRequestId: `capture-004a-req-a2-${stamp}`,
      occurredAt: t2,
      createdAt: t2,
    },
    {
      ...
      status: "received",
      externalMessageId: `capture-004a-in-a3-${stamp}`,
      clientRequestId: `capture-004a-req-a3-${stamp}`,
      occurredAt: t5,
      createdAt: t5,
    },
    {
      ...
      status: "received",
      externalMessageId: `capture-004a-in-b1-${stamp}`,
      clientRequestId: `capture-004a-req-b1-${stamp}`,
      occurredAt: t3,
      createdAt: t3,
    },
    {
      ...
      status: "sent",
      externalMessageId: `capture-004a-out-b2-${stamp}`,
      clientRequestId: `capture-004a-req-b2-${stamp}`,
      occurredAt: t4,
      createdAt: t4,
    },
  ]);
